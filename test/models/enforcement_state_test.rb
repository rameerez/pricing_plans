...
class EnforcementStateTest < ActiveSupport::TestCase
  ...
  def test_factory_creates_valid_state
    ...
    assert state.valid?
    assert_equal @org, state.billable
    assert_equal "projects", state.limit_key
  end

  def test_validation_requires_limit_key
    state = PricingPlans::EnforcementState.new(billable: @org)
  ...
  def test_polymorphic_billable_association
    ...
    # Create a different type of billable for testing
    ...
    assert_equal @org, state1.billable
    assert_equal "Organization", state1.billable_type
    assert_equal @org.id, state1.billable_id
    
    assert_equal project, state2.billable
    assert_equal "Project", state2.billable_type
    assert_equal project.id, state2.billable_id
  end
