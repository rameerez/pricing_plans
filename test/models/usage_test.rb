...
class UsageTest < ActiveSupport::TestCase
  ...
  def test_factory_creates_valid_usage
    ...
    assert usage.valid?
    assert_equal @org, usage.plan_owner
  ...
  def test_different_plan_owners_allowed
  ...
  def test_polymorphic_plan_owner_association
    ...
    assert_equal @org, usage.plan_owner
    assert_equal "Organization", usage.plan_owner_type
    assert_equal @org.id, usage.plan_owner_id
  end
